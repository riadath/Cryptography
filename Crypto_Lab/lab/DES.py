...
def DES_function(plainText,key,isEncrypt):
    ...
    for subString in substrings:
        ...
        for x in range(0,16):
            ...
            # expand the 32 bit right part to 48 bits in order to match the round key
            newRE = RE.permute( expansion_permutation )
            # xor key and expanded right side
            newRE = newRE^currentRoundKey
            # convert the 48 bit right side to 32 bit using s_box
            newRE = S_Box.substitute(newRE)
            # permute the new right side
            newRE = newRE.permute(p_box)
            # finally xor left side and final right side
            finalRE = newRE^LE
            # new value of left side and right side and then loop again
            LE = RE
            RE = finalRE
        
        # finally concat LE and RE by swapping each other
        tempBV = RE + LE
        # permute the final result with the reverse matrix of initial permutation matrix
        tempBV = tempBV.permute(initial_reverse_permutation_matrix)
        # add the result to finalAns
        finalBV += tempBV
